...
export function ThreeDImageRing({
  ...
  // 1. Tăng margin âm lên để đợi cuộn sâu hơn mới hiện (-200px)
  ...
  // Ref để xử lý logic hover mượt mà (tránh xung đột sự kiện)
  ...
  // 2. Delay việc hiển thị ảnh khi đã lọt vào khung hình
  useEffect(() => {
    if (isInView) {
      // Đợi 500ms (0.5 giây) sau khi lăn tới mới bắt đầu hiệu ứng
  ...
  // Cấu hình chuyển động opacity (mượt hơn chút nữa)
  const opacityTransition = { duration: 0.5, ease: "easeInOut" };

  // 3. Logic Hover tập trung
  const handleHover = (activeIndex) => {
    if (isDragging.current || !ringRef.current) return;

    // Hủy bỏ bất kỳ lệnh reset nào đang chờ (quan trọng để sửa lỗi giật)
    ...
    Array.from(ringRef.current.children).forEach((imgEl, i) => {
      if (activeIndex === null) {
        // Nếu không hover cái nào (được gọi từ onHoverEnd), animate tất cả về 1
        animate(imgEl, { opacity: 1 }, opacityTransition);
      } else {
        // Nếu đang hover 1 cái
        const targetOpacity = i === activeIndex ? 1 : hoverOpacity;
        animate(imgEl, { opacity: targetOpacity }, opacityTransition);
      }
    });
  };
            ...
            {showImages &&
              images.map((imageUrl, index) => (
                <motion.div
                  ...
                  // --- SỬA LOGIC HOVER ---
                  onHoverStart={() => {
                    // Gọi ngay lập tức logic hover
                    handleHover(index);
                  }}
                  onHoverEnd={() => {
                    // Thay vì reset ngay lập tức, ta chờ 100ms.
                    // Nếu chuột di sang ảnh khác, onHoverStart của ảnh đó sẽ chạy
                    // và HỦY lệnh này (clearTimeout), do đó không bị nháy.
